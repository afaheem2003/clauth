...
export async function getUserCreditLimits(userId) {
  try {
    // Get user's subscription or default to starter plan
    let subscription = await prisma.userSubscription.findUnique({
      where: { userId },
      include: { plan: true }
    });

    // If no subscription in the new table, check legacy UserCredits.subscriptionType
    if (!subscription) {
      ...
      if (userCredits?.subscriptionType) {
        // Map legacy subscription types to plan data
        const planMapping = {
          'FREE': {
        ...
        const planData = planMapping[userCredits.subscriptionType] || planMapping['FREE'];
      ...
      // Try to find starter plan in new system
      ...
      if (starterPlan) {
        ...
        return { 
          plan: { displayName: 'Starter', dailyMediumCap: 15, dailyHighCap: null, editCapPerDesign: 3 },
          monthlyMediumCredits: 120,
          monthlyHighCredits: 5
        };
      }
    ...
    return {
      ...
      monthlyMediumCredits: subscription.plan.monthlyMediumCredits,
      monthlyHighCredits: subscription.plan.monthlyHighCredits
    };
  } catch (error) {
